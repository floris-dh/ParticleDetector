...
class AlphaSpectrumAnalyser:
    ...
    def _require_loaded(self):
        if self.x is None or self.counts is None or self.sigma_counts is None:
            raise RuntimeError(
                "Spectrum data is not loaded yet. Call load_and_bin_data() first."
            )

    def _require_fit(self):
        if self.popt is None:
            raise RuntimeError(
                "Spectrum has not been fit yet. Call fit_spectrum() first."
            )

    def _require_calibration(self):
        if self.calib_popt is None or self.popt_mev is None:
            raise RuntimeError(
                "Spectrum has not been calibrated yet. Call calibrate_and_scale() first."
            )
    ...
    def load_and_bin_data(self):
        ...
        counts, bins = np.histogram(
            self.arbitrary_energy, bins=binsize, range=self.integral_range
        )
    ...
    def _moving_average(self, data, window_size=3):
        ...
        variance_smoothed = (
            np.convolve(data**2, weights, mode="full")[: len(data)] - smoothed_counts**2
        )
    ...
    def fit_spectrum(self):
        ...
        print(
            f"[{self.isotope_name}] Automatic peak detection and Gaussian curve fit..."
        )
        self._require_loaded()
        assert (
            self.x is not None
            and self.counts is not None
            and self.sigma_counts is not None
        )
        x = self.x
        counts = self.counts
        ...
        print(
            f"  Detected peaks at indices: {peaks} with energies (AU): {x[peaks]} and counts: {counts[peaks]}"
        )
    ...
    def calibrate_and_scale(self):
        ...
        if self.num_peaks < 2:
            print(
                f"[{self.isotope_name}] Not enough Peaks for Calibration. Skipping..."
            )
        ...
        if self.num_peaks >= 2:
            ...
            calib_popt, _ = curve_fit(
                linear_func, fitted_means, self.lit_energies, p0=[1e5, 0]
            )
            self.calib_popt = calib_popt
            a_slope, b_intercept = calib_popt

            print(
                f"Calibration parameters: a (slope) = {a_slope:.2e} MeV/AU, b (intercept) = {b_intercept:.3f} MeV"
            )
        ...
        for i in range(self.num_peaks):
            ...
            self.popt_mev[idx + 1] = linear_func(
                popt[idx + 1], *calib_popt
            )  # mu naar MeV
            ...
            print(
                f"  Peak {i + 1} ({self.lit_energies[i]} MeV ref) -> Calibrated: {self.popt_mev[idx + 1]:.3f} MeV | FWHM: {fwhm:.3f} MeV"
            )
    ...
    def generate_plots(self, ax, output_folder="Figures"):
        ...
        ax.plot(
            calibrated_x, counts, drawstyle="steps-mid", color="#9D593D", label="Data"
        )
...
if __name__ == "__main__":
    ...
    for i, data in enumerate(datasets):
        analysis = AlphaSpectrumAnalyser(
            csv_path=data["path"], isotope_name=data["source"]
        )
    ...
    for i, ax in enumerate(axes):
        ...
        ax.tick_params(
            colors="#BBC2C6", which="both"
        )  # Set tick colors for both major and minor ticks
