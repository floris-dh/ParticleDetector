...
INTEGRAL_CSV = r"Data\Pu239-050626_output\pulse_integrals.csv"

Ra_energies = [4.87062, 5.59031, 6.11468, 7.83346]
...
counts, bins = np.histogram(arbitrary_energy, bins=binsize)#, range=(1.4e-5, 5.5e-5))
...
init_guess = [
    800, 2.0e-5, 5e-7,  # Peak 1 Guess
    1200, 2.6e-5, 1e-6,   # Background baseline noise guess
    800, 3.1e-5, 1e-6, # Peak 3 Guess
    800, 4.5e-5, 1e-6    # Peak 4 Guess
]

bounds = (
    [0, 1.9e-5, 0, 0, 2.5e-5, 0, 0, 2.9e-5, 0, 0, 4.3e-5, 0],       # Lower bounds
    [2000, 2.1e-5, 0.01, 2000, 2.8e-5, 0.01, 2000, 3.2e-5, 0.01, 2000, 4.6e-5, 0.01]  # Upper bounds
)
...
peaks = np.array([popt[1], popt[4], popt[7], popt[10]])  # Extract the means of the four fitted Gaussians as peak positions
print("Identified peak indices:", peaks)

popt2, pcov = curve_fit(func, peaks, Ra_energies, p0=[1e5, 0], maxfev=10000)
...
peak_heights = [popt[0], popt[3], popt[6], popt[9]]  # Amplitudes of the four fitted Gaussians

# annotations that use calibrated peak positions
energies = Ra_energies
...
# Plot the Gaussian using the newly converted MeV parameters
