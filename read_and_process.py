...
def _fit_single_packet(packet):
    ...
    try:
        ...
        popt, _ = curve_fit(
            _pulse_model, t_fit, v_fit, p0=p0, bounds=bounds, method='trf',
            maxfev=2000, ftol=1e-4, xtol=1e-4, gtol=1e-4
        )
        
        chi2_red = np.sum((v_fit - _pulse_model(t_fit, *popt))**2) / (len(t_fit) - len(popt))
        if chi2_red > 1e-6:
            return None
        ...
        return {
            ...
            '_raw_t': t if chunk_id < 20 else None, 
            '_raw_v': v if chunk_id < 20 else None
        }
    except Exception:
        return None
...
class AlphaPulseProcessor:
    def __init__(self, csv_file_path, output_folder, isotope_name="Unknown", max_plots=10, max_workers=4):
        """
    ...
    def load_and_chunk_data(self):
        ...
        df_with_chunks.write_parquet(os.path.join(self.output_folder, "processed_data.parquet"))
    ...
    def process_and_fit_pulses(self, packets):
        ...
        print(f"[{self.isotope_name}] Parallel workers starting ({self.max_workers} cores) for {total_chunks} traces...")
        ...
        if not results_list:
            raise RuntimeError("No valid pulse fits were found. Check data quality and fitting parameters.")
    ...
    def _generate_diagnostic_plots(self, results_list):
        ...
        print(f"  Generating diagnostic plots for the first {self.max_plots} verified fits...")
        plots_made = 0
        ...
        for res in results_list:
            ...
            plt.plot(t_arr, v_arr, label=f"Scope Data (Chunk {c_id})", alpha=0.6, color="black")
            ...
            plt.savefig(os.path.join(self.output_folder, f"fit_plot_{c_id:04d}.png"), dpi=100, bbox_inches='tight')
            plt.close()
            plots_made += 1
    ...
    def compute_bounded_integrals(self):
        ...
        results_with_dt = self.clean_results_df.with_columns(pl.Series("dt_zero", dt_zero_arr))
        ...
        self.integrals_df = self.integrals_df.with_columns(
...
if __name__ == "__main__":
    ...
    target = "Ra226"  # Opties: "Am241", "Pu239", "Ra226"
    ...
    processor = AlphaPulseProcessor(
        ...
        max_workers=4 # Pas aan naar aantal beschikbare CPU cores
    )
